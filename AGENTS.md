...
This file is an **Agent MD**: it is written primarily for AI agents (Claude Code, GitHub Copilot, Gemini, etc.), not for humans.

- Keep this file **concise**. As a rule of thumb, aim for ~250–500 lines máximo.
- Focus on information that the AI **cannot easily infer** por sí misma:
  - estructura del proyecto y componentes principales
  - normas culturales (cómo se trabaja, qué evitar)
  - qué skills existen y cuándo deben invocarse
  - cómo ejecutar test, linters y pipelines básicos
- Evita duplicar documentación extensa: enlaza a ficheros en `docs/`, `skills/*/references/`, etc.

En proyectos grandes o monorepos, cada carpeta importante debería tener su propio `AGENTS.md` (por ejemplo, `ui/AGENTS.md`, `api/AGENTS.md`, `prowler/AGENTS.md`), y este archivo de raíz actúa como **punto de entrada y orquestador**:

- El agente que se arranca en la raíz debe **leer primero este `AGENTS.md`**.
- Después, según la tarea, puede ir al Agent MD correspondiente (UI, API, SDK, etc.).
