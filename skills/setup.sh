...
setup_copilot() {
    local target="$REPO_ROOT/.github/copilot/skills"

    # Ensure base directory for Copilot
    if [ ! -d "$REPO_ROOT/.github/copilot" ]; then
        mkdir -p "$REPO_ROOT/.github/copilot"
    fi

    # Handle existing skills dir or symlink
    if [ -L "$target" ]; then
        rm "$target"
    elif [ -d "$target" ]; then
        mv "$target" "$REPO_ROOT/.github/copilot/skills.backup.$(date +%s)"
    fi

    ln -s "$SKILLS_SOURCE" "$target"
    echo -e "${GREEN}  ✓ .github/copilot/skills -> skills/${NC}"

    # Generate per-component instructions for Copilot
    copy_agents_md_copilot "COPILOT.md"
}

copy_agents_md_copilot() {
    local target_name="$1"
    local agents_files
    local count=0
    ...
    echo -e "${GREEN}  ✓ Copied $count AGENTS.md -> $target_name for Copilot${NC}"
}
...
[ "$SETUP_COPILOT" = true ] && echo "  • GitHub Copilot: .github/copilot/skills/ + COPILOT.md"
echo ""
